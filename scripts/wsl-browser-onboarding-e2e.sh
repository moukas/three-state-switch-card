...
"$HA_PYTHON" -m homeassistant --config "$CONFIG" >"$RUN_DIR/ha-onboarding.log" 2>&1 &
HA_PID=$!
...
"$BROWSER_PYTHON" "$PROJECT/scripts/wsl-browser-onboarding.py"
tail -n 20 "$RUN_DIR/ha-onboarding.log"
